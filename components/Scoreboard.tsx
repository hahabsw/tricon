...
import { AnimatedCounter, GlassBeam, PulseRing } from "./fx/Animations";
...
export const Scoreboard = () => {
  ...
  return (
    <div className="absolute top-2 sm:top-6 left-0 right-0 z-10 flex justify-center pointer-events-none px-2">
      <div className="glass relative px-3 py-2 sm:px-8 sm:py-4 rounded-full flex items-center space-x-4 sm:space-x-12 border border-white/10 shadow-[0_10px_40px_rgba(0,0,0,0.5)] bg-black/40 backdrop-blur-xl max-w-full overflow-hidden">
        <GlassBeam />
        {turnOrder.map((pid, idx) => {
          ...
          return (
            ...
            >
              <span className="relative inline-flex w-3 h-3 sm:w-4 sm:h-4 shrink-0">
                <span
                ...
                />
                {isActive && <PulseRing color={color} size={28} duration={2000} />}
              </span>
              <div className="flex flex-col min-w-0">
                ...
                <AnimatedCounter
                  ...
                  className="font-mono text-base sm:text-2xl font-black tracking-tighter"
                  style={{
                    color: isActive ? color : "white",
                    textShadow: isActive ? `0 0 18px ${color}88` : "none",
                  }}
                />
              </div>
            </div>
