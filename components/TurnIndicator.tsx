...
export const TurnIndicator = () => {
  ...
  return (
    <div className="absolute bottom-2 sm:bottom-10 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center pointer-events-auto w-[calc(100%-1rem)] sm:w-auto max-w-md sm:max-w-none">
      ...
      >
        <div className="flex items-center justify-between gap-3 px-3 py-2 sm:px-10 sm:py-6 sm:min-w-[400px]">
          ...
          <div className="flex items-center justify-center bg-black/50 rounded-full w-12 h-12 sm:w-20 sm:h-20 border border-white/10 relative shrink-0">
            <span className={`text-xl sm:text-4xl font-black font-mono tracking-tighter tabular-nums ${isUrgent ? 'text-red-500 animate-pulse' : 'text-white'}`}>
              {turnTimeLeft}
            </span>
        ...
        {isMyTurn && (
          <div className="bg-black/40 border-t border-white/5 p-1.5 sm:p-4 flex justify-center">
            <button
              onClick={() => sendPass()}
